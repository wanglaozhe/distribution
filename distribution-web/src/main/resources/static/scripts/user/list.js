...
var pageaction = function(){
	var defaultOpt = getOpt();
    $.get('/user/list?t='+new Date().getTime(),{
    	page:defaultOpt.current_page,size:defaultOpt.items_per_page,name:$("#name").val()
    },function(result){
        if(result.success){
...
function showDetail(id){
    $.get("/user/"+id,{ts:new Date().getTime()},function(data){
        art.dialog({
            lock:true,
            opacity:0.3,
            title: "查看信息",
...
function edit(id){
    $.get("/user/edit/"+id,{ts:new Date().getTime()},function(data){
        art.dialog({
            lock:true,
            opacity:0.3,
            title: "修改",
...
function del(id){
    ...
    $.get("/user/delete/"+id,{ts:new Date().getTime()},function(result){
    	if(result.success){
	   		 alert("删除成功");
	         pageaction();
	   	}else{
