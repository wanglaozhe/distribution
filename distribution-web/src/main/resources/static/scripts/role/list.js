...
var pageaction = function(){
    $.get('/role/list?t='+new Date().getTime(),{
    	size:10,page:1,name:$("#name").val(),createdateStart:$("#createdateStart").val(),createdateEnd:$("#createdateEnd").val()
    },function(result){
    	if(result.success){
    		currentPageData = result.data.content;
            $(".pagination").pagination(result.data.totalElements, getOpt());
    	}else{
...
function showDetail(id){
    $.get("/role/"+id,{ts:new Date().getTime()},function(data){
        art.dialog({
            lock:true,
            opacity:0.3,
            title: "查看信息",
...
function edit(id){
    $.get("/role/edit/"+id,{ts:new Date().getTime()},function(data){
        art.dialog({
            lock:true,
            opacity:0.3,
            title: "修改",
...
function del(id){
    ...
    $.get("/role/delete/"+id,{ts:new Date().getTime()},function(result){
    	if(result.success){
    		 alert("删除成功");
             pageaction();
    	}else{
